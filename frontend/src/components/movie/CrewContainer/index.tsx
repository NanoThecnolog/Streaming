...
const organizeDepartments = (crewDepartment: groupedByDepartment): Department[] => {
  return Object.entries(crewDepartment)
    ...
    .sort((first, second) => {
      const firstOrder = departmentOrder[first.name] ?? 99

      const secondOrder = departmentOrder[second.name] ?? 99

      if (firstOrder !== secondOrder) {
        return firstOrder - secondOrder
      }

      return first.translatedName.localeCompare(second.translatedName, 'pt-BR')
    })
}

export default function CrewContainer({
  crewDepartment,
  limitPerDepartment = 12,
}: CrewContainerProps) {
  ...
  const handleToggleDepartment = (department: string) => {
    setExpandedDepartments((currentDepartments) => {
      const updatedDepartments = new Set(currentDepartments)
  ...
  return (
    <section className={styles.crew} aria-labelledby={titleId}>
      ...
      <div className={styles.crewContainer}>
        {departments.map((department, index) => {
          ...
          const visibleMembers = isExpanded
            ? department.members
            : department.members.slice(0, normalizedLimit)

          const hasMoreMembers = department.members.length > normalizedLimit

          const hiddenMembers = department.members.length - normalizedLimit

          const departmentId = `${titleId}-department-${index}`
          ...
          return (
            ...
            >
              <header className={styles.departmentHeader}>
                <h3 id={`${departmentId}-title`} className={styles.departmentTitle}>
              ...
              <div id={departmentId} className={styles.departmentCrew}>
                {visibleMembers.map((member) => (
                  <Crew key={`${member.id}-${member.job}`} crew={member} />
                ))}
              </div>
              ...
              {hasMoreMembers && (
                <button
                  ...
                  onClick={() => {
                    handleToggleDepartment(department.name)
                  }}
                  aria-expanded={isExpanded}
                  aria-controls={departmentId}
                >
                  <span>{isExpanded ? 'Mostrar menos' : `Ver mais ${hiddenMembers}`}</span>

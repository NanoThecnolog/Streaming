...
const NewTop = ({
  ...
  const genres = useMemo(() => {
    ...
    if (tmdbData?.genres?.length) {
      return tmdbData.genres.map((genre) => translations[genre.name] ?? genre.name).join(' - ')
    }
  ...
  }, [disableVideoOnFirst, id, isActive, isDesktop, videoFailed])
  ...
  useEffect(() => {
    ...
    const createTmdbImage = (path: string | null | undefined): string | null => {
      if (!path) return null
    ...
    const loadTmdbData = async () => {
      try {
        if (type === 'movie') {
          const cachedData = allData.find((item) => item.id === id)

          const data = cachedData ?? (await tmdb.fetchMovieDetails(id))
        ...
        const cachedData = serieData.find((item) => item.id === id)

        const data = cachedData ?? (await tmdb.fetchSeriesDetails(id))
  ...
  return (
    ...
    >
      <div className={`${styles.bannerImage} ${showVideo ? styles.hidden : ''}`} aria-hidden="true">
        <img src={backgroundImage} alt="" loading={isActive ? 'eager' : 'lazy'} />
      </div>
      ...
      <div className={styles.content}>
        <div className={`${styles.information} ${showVideo ? styles.playing : ''}`}>
          <span className={styles.featuredLabel}>Destaque FlixNext</span>
          ...
          <div className={styles.metadata}>
            ...
            {genres && <span className={styles.genres}>{genres}</span>}
          ...
          <div className={styles.actions}>
            ...
            <button
              ...
              onClick={(event) => {
                handleActionClick(event, handleMoreInfo)
              }}
            >
        ...
        {showVideo && (
          <div className={styles.volumeControl} onClick={(event) => event.stopPropagation()}>
            <label className={styles.visuallyHidden} htmlFor={`volume-${type}-${id}`}>
              Volume do trailer
            </label>

...
export default function ForgetPass({ handleClose }: ForgetPassProps) {
  ...
  const sendRedefineToken = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      toast.info('Se o email estiver cadastrado, enviaremos o link de recuperação.')

      handleClose()
    } catch (error: unknown) {
      console.error('Erro ao solicitar a recuperação da senha:', error)

      if (isAxiosError<RecoverTokenErrorResponse>(error)) {
        const message = error.response?.data?.error ?? error.response?.data?.message

        toast.error(message ?? 'Não foi possível enviar o link. Tente novamente.')
    ...
    <div className={styles.overlay} role="presentation" onMouseDown={handleOverlayClick}>
      ...
      >
        ...
        <header className={styles.header}>
          ...
          <h2 id="recover-modal-title">Esqueceu sua senha?</h2>
        ...
        <form className={styles.form} onSubmit={sendRedefineToken} noValidate>
          <div className={styles.field}>
            ...
            {email.length > 0 && !emailIsValid && (
              <span className={styles.fieldError}>Digite um endereço de e-mail válido.</span>
            )}
          </div>
        ...
        <footer className={styles.security}>
          ...
          <span>O link de recuperação possui tempo limitado.</span>
        </footer>
      </section>
    </div>
  )

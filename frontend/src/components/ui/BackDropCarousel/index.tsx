...
export default function BackDropCarousel({ title /*cardPerContainer*/ }: BaseProps) {
  ...
  useEffect(() => {
    ...
    const movieMap = new Map(allData.map((movie) => [movie.id, movie]))
    const serieMap = new Map(serieData.map((serie) => [serie.id, serie]))
  ...
  useEffect(() => {
    ...
    const fetchProgress = async (tracking: MapTrackingProps): Promise<number | null> => {
      try {
        ...
        const progressEntry = data.result?.find((item) => {
          if (item.tmdbID !== Number(tracking.id) || item.mediaType !== tracking.type) {
            return false
          }
          ...
          return item.season === tracking.season && item.episode === tracking.episode
        })

        return progressEntry?.progress ?? null
      } catch (error) {
        if (!axios.isCancel(error)) {
          debug.error(`Erro ao buscar progresso de ${tracking.id}`, error)
        }
    ...
    const getRuntime = async (item: CarouselCard): Promise<number | null> => {
      if (item.tracking.type === 'movie') {
        return 'runtime' in item.card ? (item.card.runtime ?? null) : null
      }

      if (item.tracking.season === undefined || item.tracking.episode === undefined) {
        return null
      }

      const episodes = await tmdb.fetchEpisodeData(item.card.id, item.tracking.season)

      const episode = episodes?.find((episode) => episode.episode_number === item.tracking.episode)
      ...
      const results = await Promise.allSettled(
        baseCards.map(async (item) => {
          ...
          return {
            ...item,
            progress,
            percentage: runtime ? calculateVideoProgress(progress ?? 0, runtime) : 0,
          }
        }),
      )
      ...
      >
        {cards.map(({ card, tracking, progress, percentage }) => {
          ...
          if (typeGuard(card)) {
            ...
            return (
              <SwiperSlide key={`tv-${card.id}`} className={styles.slide}>
                <div className={styles.cardWrapper}>
                  <Link href={`/watch/serie?${link}`} className={styles.link}>
                    <img
                    ...
                    >
                      <div
                        ...
                        style={{ width: `${percentage > 95 ? 100 : percentage}%` }}
                      />
                    </div>
                  </Link>
                </div>
                ...
                <div className={styles.cardWrapper}>
                  <Link href={`/${path}/${card.id}`} className={styles.link}>
                    ...
                    >
                      <div
                        className={styles.progressFill}
                        style={{ width: `${percentage >= 92 ? 100 : percentage}%` }}
                      />
                    </div>
                  </Link>
                </div>

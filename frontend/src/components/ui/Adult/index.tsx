...
export default function ContentRating({ faixa }: ContentRatingProps) {
  const normalizedRating = faixa?.trim().toLocaleUpperCase('pt-BR')

  if (!normalizedRating) return null

  const rating = classification.find(
    ({ etaria, label }) =>
      etaria.toLocaleUpperCase('pt-BR') === normalizedRating || label === normalizedRating,
  )
  ...
  return (
    ...
    >
      <span aria-hidden="true">{rating.label}</span>
    </div>
  )
}

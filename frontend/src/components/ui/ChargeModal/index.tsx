...
import { ChargeData, ChargeDetailResponse, History, Item } from '@/@types/efi/chargeEfi'
...
type ChargeStatus = 'paid' | 'waiting' | 'unpaid' | 'canceled' | 'refunded' | 'expired' | string
...
const getPaymentMethodLabel = (method: ChargeData['payment']['method']): string => {
...
export default function ChargeModal({
  ...
  const renderLoading = () => {
    return (
      <div className={styles.loadingArea}>
        <Loader2 className={styles.loadingIcon} size={38} />
    ...
    return (
      <div className={styles.paymentDetailsGrid}>
        <div className={styles.paymentDetail}>
          ...
          <strong>{formatedDate(billet.expire_at)}</strong>
        </div>

        <div className={styles.paymentDetail}>
          <span>Código de barras</span>

          <strong className={styles.breakableValue}>{billet.barcode || 'Não informado'}</strong>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className={styles.paymentDetailsGrid}>
        ...
        <div className={styles.paymentDetail}>
          ...
          <strong>
            {creditCard.installments}x de {formatCurrency(creditCard.installment_value)}
          </strong>
        </div>
      </div>
    )
  ...
  const renderItems = () => {
    if (!charge?.items.length) {
      return <p className={styles.emptyMessage}>Nenhum item encontrado.</p>
    }
      ...
      <div className={styles.itemsList}>
        {charge.items.map((item, index) => (
          <div className={styles.itemRow} key={`${item.name}-${index}`}>
            ...
            <strong className={styles.itemTotal}>{formatCurrency(getItemTotal(item))}</strong>
          </div>
        ))}
      </div>
    )
  ...
  const renderHistory = (history: History[]) => {
    if (!history.length) {
      return <p className={styles.emptyMessage}>Nenhum histórico disponível.</p>
    }
    ...
    return (
      <div className={styles.timeline}>
        {orderedHistory.map((historyItem, index) => (
          <div className={styles.timelineItem} key={`${historyItem.created_at}-${index}`}>
            <div className={styles.timelineMarker} />

            <div className={styles.timelineContent}>
              <strong>{historyItem.message}</strong>

              <span>{formatedDate(historyItem.created_at)}</span>
            </div>
          </div>
        ))}
      </div>
          ...
          <>
            <header className={styles.modalHeader}>
              ...
              <div className={styles.headerContent}>
                ...
                <h2 id="charge-modal-title">#{charge.charge_id}</h2>

                <p>Criada em {formatedDate(charge.created_at)}</p>
              </div>
            ...
            <section className={styles.summaryGrid}>
              ...
              <article className={styles.summaryCard}>
                ...
                <div>
                  ...
                  <strong>{getPaymentMethodLabel(charge.payment.method)}</strong>
                </div>
              </article>
              ...
              <article className={styles.summaryCard}>
                ...
                <div>
                  ...
                  <strong>#{charge.subscription.subscription_id}</strong>
                </div>
              </article>
            </section>
              ...
              <div className={styles.paymentBox}>
                <div className={styles.paymentHeader}>
                  ...
                  <span className={styles.paymentDate}>
                    {formatedDate(charge.payment.created_at)}
                  </span>
                </div>

                {method === 'banking_billet' && renderBankingBillet()}

                {method === 'credit_card' && renderCreditCard()}

                {paymentMessage && <div className={styles.paymentMessage}>{paymentMessage}</div>}

                {charge.payment.message && (
                  <div className={styles.paymentMessage}>{charge.payment.message}</div>
                )}
            ...
            <section className={styles.section}>
              ...
              <div className={styles.infoGrid}>
                ...
                <div className={styles.infoCard}>
                  ...
                  <strong>{charge.customer.phone_number}</strong>
                </div>
              </div>
            </section>
            ...
            <footer className={styles.modalActions}>
              <button type="button" className={styles.modalCancel} onClick={closeChargeModal}>
                Fechar
              </button>
            </footer>
          </>

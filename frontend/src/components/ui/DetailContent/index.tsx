...
export default function Details({ duration, runtime, releaseDate, language }: DetailsProps) {
  const durationLabel = getDurationLabel(duration, runtime)

  const releaseYear = getReleaseYear(releaseDate)

  const languageLabel = language ? languageLabels[language] : null

  const details = [durationLabel, releaseYear, languageLabel].filter((detail): detail is string =>
    Boolean(detail),
  )
  ...
  return (
    <div className={styles.movieDetail}>
      <ul className={styles.detailsList} aria-label="Informações do filme">
        {details.map((detail) => (

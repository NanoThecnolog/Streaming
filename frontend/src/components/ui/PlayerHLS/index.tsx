...
function PlayerHLS({
  ...
  const loadSubtitleTracks = () => {
    ...
    for (let i = 0; i < tracks.length; i++) {
      ...
      if (label.includes('forced')) {
        type = 'forced'
      } else if (label.includes('full') || label.includes('sdh') || label.includes('complete')) {
        type = 'full'
      }
    ...
    requestAnimationFrame(() => {
      const currentAudioTracks =
        hlsRef.current?.audioTracks.map((track, i) => ({
          id: i,
          name: track.name || track.lang || `Audio ${i}`,
          lang: track.lang || 'unknown',
        })) || []
      applySavedPreferences(currentAudioTracks, parsedTracks)
    })
  }
  ...
  const selectSubtitleTrack = (trackId: number | null) => {
    ...
    if (trackId === null) {
      ...
      savePlayerPreferences({
        ...
        subtitleTrack: trackId !== null ? subtitleTracks.find((i) => i.id === trackId) : null,
  ...
  useEffect(() => {
    ...
    const resumePlayback = async () => {
      try {
        ...
        debug.error('Não foi possível continuar a reprodução', error)
      }
    }
  ...
  const saveProgress = useCallback(async () => {
    ...
    const epData = extractSeasonEpisode(src)
    const completedPercentage = epData ? 0.95 : 0.9
  ...
  const applySavedPreferences = (tracks: AudioTrack[], subtitles: SubtitleTrack[]) => {
    ...
    if (hlsRef.current && preferences.audioLanguage) {
      const audioIndex = tracks.findIndex((track) => {
        ...
        return normalizeLanguage(track.lang) === normalizeLanguage(preferences.audioLanguage!)
      })
    ...
    <>
      <div
        ...
        className={`${styles.container}
                    ${isFullscreen && !showControls && isPlaying ? styles.hideCursor : ''}
                `}
        ...
        {src && (
          <>
            ...
            {tapFeedback.visible && tapFeedback.side && (
              <div className={`${styles.tapFeedback} ${styles[tapFeedback.side]}`}>
                <div className={styles.content}>
                  <span>{tapFeedback.side === 'left' ? '-10s' : '+10s'}</span>
                </div>
              </div>
            )}
            ...
            {!isVideoLoading && (
              <>
                <div
                  className={`${styles.videoPlayButton} ${showPlayButton ? styles.visible : styles.hidden}`}
                >
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                  ...
                  >
                    {!isVideoLoading && isPlaying ? <FaPause /> : <FaPlay />}
                  </button>
                </div>

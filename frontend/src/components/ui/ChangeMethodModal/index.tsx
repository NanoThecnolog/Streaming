...
const isEfiTokenResult = (
    result: unknown,
): result is EfiTokenResult => {
    if (
    ...
    return (
        typeof tokenResult.payment_token === 'string' &&
        typeof tokenResult.card_mask === 'string'
    )
}
...
const ChangeMethodModal = ({ closeModal, before, setNewMethod }: ModalProps) => {
    ...
    const methodContent = useMemo(() => {
        if (newMethod === 'credit') {
            return {
                ...
                description:
                    'Preencha os dados do cartão que será usado nas próximas cobranças.',
        ...
        return {
            ...
            description:
                'As próximas cobranças serão emitidas por boleto bancário.',
    ...
    const createCreditToken =
        async (): Promise<EfiTokenResult> => {
            if (!credit) {
                throw new Error(
            ...
            if (cpf.length !== 11) {
                throw new Error(
                    'O CPF do titular não está cadastrado corretamente.',
                )
            }
            ...
            if (!accountId || !environment) {
                throw new Error(
                    'A configuração de pagamento está indisponível.',
                )
            }

            const expiration =
                credit.expiration.replace(/\D/g, '')

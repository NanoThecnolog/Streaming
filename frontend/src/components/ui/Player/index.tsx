...
function Player({ loading, shared, src, title, isSerie }: MoviePlayerProps) {
  ...
  if (isDrive === null) return <Spinner />

  if (isDrive === true && shared === null) return <Spinner />

  if ((isDrive === true && shared === false) || (isDrive === false && videoError)) {
    debug.log(
      `apresentando noFile. isDrive: ${isDrive}, shared: ${shared}, videoError: ${videoError}`,
    )

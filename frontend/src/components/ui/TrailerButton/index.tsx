...
const getMostRecentTrailer = (
    videos: VideoProps[],
): VideoProps | null => {
    return videos
        .filter(video => (
            video.type?.toLocaleLowerCase('pt-BR') ===
            'trailer' &&
            Boolean(video.key?.trim())
        ))
        .reduce<VideoProps | null>(
            (mostRecent, current) => {
                if (!mostRecent) return current

                return getPublishedTimestamp(current) >
                    getPublishedTimestamp(mostRecent)
                    ? current
                    : mostRecent
            },
            null,
...
export default function TrailerButton({
    ...
    const selectedTrailer = getMostRecentTrailer(
        trailer.results ?? [],
    )

...
const PutTV = ({ tmdbid }: PutTVProps) => {
  ...
  useEffect(() => {
    ...
    const loadSerie = async () => {
      ...
      try {
        ...
        if (!response) {
          toast.warning('Série não encontrada no catálogo.')
        ...
        const streamingGenres = tmdbDetails ? getAvailableStreamingGenres(tmdbDetails) : []
        ...
        debug.log('Erro ao carregar série:', error)
  ...
  const handleGenres = (event: ChangeEvent<HTMLSelectElement>) => {
    const genero = Array.from(event.currentTarget.selectedOptions, ({ value }) => value)
  ...
  const handleAddSeason = () => {
    updateSeasons((seasons) => {
      const greatestSeasonNumber = seasons.reduce((greatest, season) => {
  ...
  const handleRemoveEpisode = (seasonIndex: number, episodeIndex: number) => {
    updateSeasons((seasons) => {
      return seasons.map((season, index) => {
        ...
        return {
          ...season,
          episodes: season.episodes.filter((_, index) => {
  ...
  const matchGenres = (storedGenres: string[] | undefined, availableGenres: string[]): string[] => {
    if (!Array.isArray(storedGenres)) return []

    const genreMap = new Map(availableGenres.map((genre) => [normalizeGenre(genre), genre]))
  ...
  const validateSerie = (): boolean => {
    ...
    if (serieData.season.length === 0) {
      toast.warning('A série precisa ter pelo menos uma temporada.')
    ...
    for (const season of serieData.season) {
      if (season.episodes.length === 0) {
        toast.warning(`A temporada ${season.s} não possui episódios.`)
      ...
      const invalidEpisode = season.episodes.some(({ ep, src }) => {
        return ep <= 0 || !src.trim()
      })

      if (invalidEpisode) {
        toast.warning(`Verifique os episódios da temporada ${season.s}.`)
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await mongoService.updateSerie(tmdbid, payload)

      setSerieData(payload)

      toast.success('Série atualizada com sucesso!')
    } catch (error) {
      debug.log('Erro ao atualizar série:', error)
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      <div className={styles.formRow}>
        <div>
          ...
          <div className={styles.formItem}>
            ...
            {loadingSerie && <small>Buscando dados no TMDB...</small>}
          </div>
      ...
      <div className={styles.buttonContainer}>
        <button
          type="submit"
          disabled={saving || loadingSerie || serieData.tmdbID <= 0 || !serieData.title.trim()}
        >

...
import { MdAdd, MdDeleteOutline, MdEdit, MdLocalMovies, MdSearch } from 'react-icons/md'
...
const MovieDash = ({ id }: MovieDashProps) => {
  ...
  const [action, setAction] = useState<MovieAction>(initialId ? 'put' : 'create')

  const [idInput, setIdInput] = useState(initialId ? String(initialId) : '')

  const [selectedMovieId, setSelectedMovieId] = useState<number | undefined>(initialId)
  ...
  const renderForm = () => {
    switch (action) {
      case 'create':
        return <Create key={selectedMovieId} tmdbid={selectedMovieId || 0} />

      case 'put':
        return hasMovieId ? (
          <Put key={selectedMovieId} tmdbid={selectedMovieId} />
        ) : (
      ...
      case 'delete':
        return hasMovieId ? (
          <Delete key={selectedMovieId} tmdbid={selectedMovieId} />
        ) : (
  ...
  return (
    <div className={styles.container}>
      <section className={styles.searchPanel}>
        ...
        <form className={styles.searchForm} onSubmit={handleSearch}>
          <div className={styles.inputGroup}>
            <label htmlFor="movie-id">ID do filme</label>
            ...
            <input
              ...
              aria-describedby={idError ? 'movie-id-error' : undefined}
              onChange={(event) => {
            ...
            {idError && (
              <span id="movie-id-error" className={styles.error}>
                {idError}
              </span>
            )}
          </div>
      ...
      <nav className={styles.navContainer} aria-label="Ações de gerenciamento de filmes">
        ...
        <ul className={styles.actionList}>
          {ACTIONS.map((item) => {
            ...
            const isDisabled = item.requiresId && !hasMovieId
      ...
      <section className={styles.formContainer}>
        <div className={styles.formContent}>{renderForm()}</div>
      </section>
    </div>
  )
}

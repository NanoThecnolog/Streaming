    ...
    const getTMDBDetails = async () => {
      ...
      try {
        ...
        const tmdbGenres = dataTMDB.genres.map((genre) => {
          return genre.name === 'Thriller' ? 'Suspense' : genre.name
        })
        ...
        setMovieData((previousData) => ({
          ...
          duration: dataTMDB.runtime ? minToHour(dataTMDB.runtime) : '',
          genero: mergeStreamingGenres(tmdbGenres, streamingGenres),
        }))
      } catch (error) {
        if (!isCurrentRequest) return

        debug.log('Erro ao buscar dados do filme no TMDB:', error)

        toast.error('Não foi possível buscar os dados do filme no TMDB.')
      } finally {
        if (isCurrentRequest) {
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      <div className={styles.selectedContent}>
        ...
        <strong>
          {loadingTMDB ? 'Buscando dados no TMDB...' : movieData.title || 'Filme não encontrado'}
        </strong>
          ...
          <div className={styles.formItem}>
            ...
            >
              {classification.map((faixa) => (
                <option key={faixa.etaria} value={faixa.etaria}>
                  {faixa.etaria}
                </option>
              ))}
            </select>
      ...
      <div className={styles.buttonContainer}>
        <button type="submit" disabled={loading || loadingTMDB || !movieData.title.trim()}>

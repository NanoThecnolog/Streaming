...
export default function Top({ width, cards }: TopProps) {
  ...
  useEffect(() => {
    const getImages = async () => {
      ...
      if (data) {
        ...
        const backdropUrl = await tmdb.fetchMovieBackDrop(card.tmdbId)
        const posterUrl = await tmdb.fetchMoviePoster(card.tmdbId)
        setTMDBMovie(movie)
        setTMDBImages({ backdrop: backdropUrl, poster: posterUrl })
      }
    }
  ...
  return (
    <div
      className={`${styles.top_container} ${styles[fade]}`}
      style={{ backgroundImage: `url(${getBackgroundImage()})` }}
      id="inicio"
    >
      <div className={styles.image_container} id="inicio">
        <div className={styles.left_side}>
          <h1 className={styles.titulo_principal}>{card.title ?? card.title}</h1>
          {card.subtitle && <h3 className={styles.subtitulo_principal}>{card.subtitle}</h3>}

...
const hiddenProductionPlanId = 'b7c7a2d4-9b6c-4f9c-9b0e-123456789abc'
...
const Prices = ({ plans, setPlanSelected }: PricesProps) => {
  ...
  const visiblePlans = useMemo(() => {
    return [...plans]
      ...
      .sort((firstPlan, secondPlan) => {
        const firstType = isPlanType(firstPlan.type) ? planMetadata[firstPlan.type] : null

        const secondType = isPlanType(secondPlan.type) ? planMetadata[secondPlan.type] : null

        const firstMonthlyPrice = firstType ? firstPlan.price / firstType.months : firstPlan.price
  ...
  return (
    <div className={styles.prices}>
      <div className={styles.plansGrid} aria-label="Planos de assinatura disponíveis">
        {visiblePlans.map((plan) => {
          const type = isPlanType(plan.type) ? plan.type : 'mensal'

          const metadata = planMetadata[type]
          const monthlyPrice = plan.price / metadata.months

          const discount = desconto[type] ?? 0
          const isRecommended = type === recommendedPlan

          return (
            <article
              key={plan.id}
              className={`${styles.planCard} ${isRecommended ? styles.recommendedCard : ''}`}
            >
              {isRecommended && <div className={styles.recommendedBadge}>Mais recomendado</div>}

              <div className={styles.planHeader}>
                <div>
                  <p className={styles.planPeriod}>Plano {type}</p>

                  <h3>{plan.name}</h3>
                </div>

                {discount > 0 && <span className={styles.discount}>Economize {discount}%</span>}
              </div>
              ...
              <p className={styles.billing}>
                {metadata.periodLabel} de <strong>{formatPrice(plan.price)}</strong>
              </p>

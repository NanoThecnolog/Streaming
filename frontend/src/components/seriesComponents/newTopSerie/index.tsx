...
export default function NewTopSerie({ width, id, isActive = false }: TopSerieProps) {
  ...
  return (
    <div className={styles.topContainer} id="inicio">
      ...
      <div
        className={`${styles.bannerImage} ${styles.fadeIn} ${width > 915 && showVideo ? styles.hidden : ''}`}
      >
        <img src={`${getBackgroundImage()}`} alt="banner" />
      </div>
      <div className={styles.overlay}>
        <div className={`${styles.leftSide} ${videoOn ? styles.playing : ''}`}>
          <h1 className={styles.tituloPrincipal}>{card.title.toUpperCase()}</h1>
          {card.subtitle && <h3 className={styles.subtituloPrincipal}>{card.subtitle}</h3>}
      ...
      {width > 915 && (
        <video
          ...
          onEnded={() => {
            ;(setShowVideo(false), setVideoOn(false))
          }}
          onPlay={() => animateInfo()}
        />
      )}

...
const getTitleKeywords = (title: string): Set<string> => {
  ...
  return new Set(words.filter((word) => word.length >= 3 && !TITLE_STOP_WORDS.has(word)))
}
...
const getRelatedItems = <T>(
  ...
  return items
    ...
    .map((item) => {
      ...
      const sharedGenres = itemGenres.filter((genre) => referenceGenreSet.has(genre))

      const allReferenceGenresMatch =
        referenceGenres.length > 0 && sharedGenres.length === referenceGenres.length

      const firstGenreMatches =
        referenceFirstGenre !== undefined && itemGenres[0] === referenceFirstGenre

      const heroGenreMatches = sharedGenres.filter((genre) => HERO_GENRES.has(genre)).length

      const popularity = popularityById.get(itemId) ?? 0

      const keywordCoverage =
        referenceKeywords.size > 0 ? matchingKeywords / referenceKeywords.size : 0

      const isRelatedTitle =
        itemTitle.includes(referenceTitle) || referenceTitle.includes(itemTitle)
...
const createPopularityMap = (
  ...
  return new Map(items.map((item) => [item.id, item.popularity ?? 0]))
}
...
export const getRelatedCards = (
  ...
  return getRelatedItems(movie, movies, createPopularityMap(allData ?? []), {
...
export const getRelatedSerieCards = (
  ...
  return getRelatedItems(serie, series, createPopularityMap(allData ?? []), {
    getId: (item) => item.tmdbID,
    getTitle: (item) => item.title,
    getGenres: (item) => item.genero ?? [],
  })
...
const isSeriesContent = (item: CatalogContent): item is SeriesProps => {
...
export const getRelatedContent = (
  ...
  const popularityById = new Map<number, number>([
    ...movieData.map((item): [number, number] => [item.id, item.popularity ?? 0]),
    ...seriesData.map((item): [number, number] => [-item.id, item.popularity ?? 0]),
  ])
  ...
  const rankedItems = getRelatedItems<CatalogContent>(
    ...
    popularityById,
    {
      getId: (item) => (isSeriesContent(item) ? -item.tmdbID : item.tmdbId),
      getTitle: (item) => item.title,
      getGenres: (item) => item.genero ?? [],
  ...
  for (const item of rankedItems) {
    ...
    const key = isSeriesContent(item) ? `series-${item.tmdbID}` : `movie-${item.tmdbId}`
...
export const filterCards = <T extends CardsProps | SeriesProps>(
  ...
  return items.filter((item) =>
    item.genero?.some((genre) => normalizeText(genre) === normalizedSection),
  )
}

...
class Drive {
  ...
  public async checkFile(url: string) {
    ...
    try {
      ...
      if (status === 404) throw new DriveFileError('FILE_NOT_FOUND')

      if (status === 403) throw new DriveFileError('FILE_PRIVATE')
  ...
  public async verifyAllDataFiles() {
    ...
    const errors = results.filter((item): item is ErrorProps => item !== null)
        ...
        season.episodes.map((episode) =>
          this.limit(async (): Promise<ErrorProps | null> => {
            try {
              ...
              debug.error(`Erro no episódio ${episode.ep} | T${season.s} | ${card.title}`, reason)

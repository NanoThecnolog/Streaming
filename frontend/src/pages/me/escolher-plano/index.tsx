...
import { CreditCardData, PaymentMethod, PaymentStatus, PersonalData, PlanProps } from '@/@types/payment'
...
const isPaymentApiResponse = (response: ReactivationPaymentResponse): response is PaymentApiResponse => {
...
export default function ChoosePlanPage({
    ...
    useEffect(() => {
        ...
        const queryPlanId = Array.isArray(router.query.id)
            ? router.query.id[0]
            : router.query.id
        ...
        const defaultPlan =
            plans.find(plan => plan.type === 'mensal') ??
            plans[0]
    ...
    const handlePayment = async () => {
        if (!user || !selectedPlan) {
            toast.warning(
                'Não foi possível identificar o usuário ou plano.',
            )
            return
        }
        ...
        const result =
            await processPayment<ReactivationPaymentResponse>({
    ...
    return (
        <>
            <Head>
                <title>
                    Reativar assinatura | FlixNext
                </title>

                <meta
                    name="description"
                    content="Escolha um plano e reative sua assinatura."
                />
            </Head>
...
export const getServerSideProps:
    GetServerSideProps<PageProps> = async ({ req }) => {
        ...
        try {
            ...
            const response = await axios.get<PlanProps[]>(

...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const data = req.body
  if (!data.customer) return res.status(400).json({ error: 'Dados do cliente inválidos' })
  ...
  try {
    ...
    if (!Validate.cpf(customer.cpf)) return res.status(400).json({ message: 'CPF inválido!' })
    ...
    if (!expireAt) return res.status(500).json({ error: 'Erro ao gerar data de vencimento.' })
    ...
    const response = await apiSub.post(`/subscription/${userId}/reactivate`, body)

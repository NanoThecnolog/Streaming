  ...
  const fetchData = async () => {
    ...
    results.forEach((result, index) => {
      if (result.status === 'fulfilled') {
        ...
        index === 0
          ? debug.error('Erro ao buscar filme:', error)
          : debug.error('Erro ao buscar série', error)
      }
    })
  }
  useEffect(() => {
    ...
    <>
      ...
      <div className={styles.container}>
        <div>
          ...
          <button onClick={() => verify(`movie`)}>
            {loading ? 'Testando...' : 'Testar filmes'}
          </button>
          <button onClick={() => verify(`tv`)}>{loading ? 'Testando...' : 'Testar series'}</button>
          {errors && errors.length > 0 ? (
            <button onClick={() => downloadErrors(errors)}>baixar Erros</button>
          ) : (
            ''
          )}
            ...
            <div className={styles.mongoContainer}>
              ...
              {movies &&
                movies
                  ...
                  .map((movie, index) => <div key={index}>{movie.title}</div>)}
            </div>
            <div className={styles.mongoContainer}>
              <h2>Series</h2>
              {series &&
                series
                  .sort((a, b) => b.index - a.index)
                  .map((serie, index) => <div key={index}>{serie.title}</div>)}
            </div>
          </div>
        </div>
      </div>

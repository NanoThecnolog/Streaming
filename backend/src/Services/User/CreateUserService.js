"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CreateUserService = void 0;
const dotenv_1 = __importDefault(require("dotenv"));
dotenv_1.default.config();
const bcrypt_1 = require("bcrypt");
const prisma_1 = __importDefault(require("../../prisma"));
const apiMessenger_1 = require("../../Utils/apiMessenger");
const badRequestExtend_1 = require("../../Utils/badRequestExtend");
const validator_1 = require("validator");
class CreateUserService {
    execute(_a) {
        return __awaiter(this, arguments, void 0, function* ({ name, email, birthday, password, cpf }) {
            const userExiste = yield prisma_1.default.user.findFirst({
                where: { email }
            });
            if (userExiste) {
                throw new badRequestExtend_1.BadRequestError("Email já cadastrado.");
            }
            if (!(0, validator_1.isEmail)(email)) {
                throw new badRequestExtend_1.BadRequestError("Email inválido");
            }
            const passwordHash = yield (0, bcrypt_1.hash)(password, 8);
            const user = yield prisma_1.default.user.create({
                data: {
                    name,
                    email,
                    birthday,
                    password: passwordHash,
                    cpf
                }, select: {
                    id: true,
                    name: true,
                    email: true
                }
            });
            if (user) {
                const activateLink = `${process.env.WEBSITE_LINK}/ativando-conta/${user.id}`;
                const qrCode = "data:image/png;base64,";
                let responses;
                try {
                    const systemNotification = yield apiMessenger_1.apiEmail.post('/system/new/user', {
                        name: user.name,
                        email: user.email,
                        birthday: birthday,
                        password: password,
                    });
                    const sendEmailToUser = yield apiMessenger_1.apiEmail.post('/user/new', {
                        name: user.name,
                        email: user.email,
                        activateLink,
                        qrCode
                    });
                    responses = {
                        user,
                        systemNotify: systemNotification.data.data.accepted.length > 0 ? 'email enviado' : 'email não enviado',
                        userNotify: sendEmailToUser.data.data.accepted.length > 0 ? 'email enviado' : 'email não enviado',
                    };
                }
                catch (err) {
                    console.log(err);
                    throw new badRequestExtend_1.BadRequestError("Erro ao enviar email");
                }
                return responses;
            }
            else {
                throw new badRequestExtend_1.BadRequestError("Usuário não criado.");
            }
        });
    }
}
exports.CreateUserService = CreateUserService;

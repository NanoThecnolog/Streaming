...
class CreateUserService {
    async execute({ name, email, birthday, password }: UserRequest) {
        ...
        if (user) {
            ...
            const systemNotification = await apiEmail.post('/system/new/user', {
                name: user.name,
                email: user.email,
                birthday: birthday,
                password: password,
            })
            //console.log("notificação para o sistema", systemNotification)
            const sendEmailToUser = await apiEmail.post('/user/new', {
                name: user.name,
                email: user.email,
                activateLink,
                qrCode
            })
            //console.log("Notificação para usuário", sendEmailToUser)
            const responses = {
                user,
                systemNotify: systemNotification.data.accepted.length > 0 ? 'email enviado' : 'email não enviado',
                userNotify: sendEmailToUser.data.accepted.length > 0 ? 'email enviado' : 'email não enviado'
            }
            return responses

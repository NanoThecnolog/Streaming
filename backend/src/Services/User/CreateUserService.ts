...
class CreateUserService {
    async execute({ name, email, birthday, password }: UserRequest) {
        ...
        try {
            const ActiveLink = `http://localhost:3333/ativar?email=${email}`
            ...
            const info = await transporter.sendMail({
                ...
                text: `Olá, ${user.name}! Esse é um email automático. Por favor, não responda. Obrigado por criar uma conta no FlixNext! Por favor, clique no link para ativar sua conta: ${ActiveLink}`,
                html: `
